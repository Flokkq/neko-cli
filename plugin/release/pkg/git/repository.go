...
func LastCommit() (string, error) {
	log.PluginV(log.Exec, "Fetching last commit: "+
...
func TotalCommits() (string, error) {
	log.PluginV(log.Exec, "Counting total commits: "+
...
func FilesCount() (int, error) {
	log.PluginV(log.Exec, "Counting tracked files: "+
...
func RepoSize() (string, error) {
	log.PluginV(log.Exec, "Calculating repository size: "+
...
func DeleteGithubRelease(tag string, token string) error {
	...
	if resp.StatusCode == http.StatusNotFound {
		log.PluginV(log.Exec, fmt.Sprintf("GitHub release for tag %s not found (nothing to delete)", tag))
		return nil
	}
	...
	log.PluginV(log.Exec, fmt.Sprintf("Deleted GitHub release for tag %s", tag))
	return nil
}
